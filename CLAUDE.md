...
- [helper/statistika.js](src/bot/helper/statistika.js) — hidden `/statshour` command. Not registered in `bot.setMyCommands`, so it does not appear in Telegram's command-menu autocomplete. Non-authorised users fall through to `start()`, so the command's existence is not observable from the outside. Output is yesterday's full day (Asia/Tashkent, UTC+5), 24 hourly buckets `00-01` … `23-00`, for both bot signups (from `Users.createdAt`) and clicks (from `STATS_API_URL` timeseries endpoint, defaults to `https://api.logistic-dev.coachingzona.uz/v1/stats/timeseries`, path `STATS_API_PATH` default `/v1/post/all`). Timezone shift is applied at query time only — `createdAt` is stored as UTC and not rewritten.
- [helper/broadcast.js](src/bot/helper/broadcast.js) — hidden `/broadcast` command. Sends the `start()` greeting variant (Mini App button only, no dispatcher button) to every user in `Users`, personalised with the first token of their `full_name`. Throttled at ~20 msg/sec; handles Telegram `429` with `retry_after`; per-user failures are logged and counted, then a summary (`Yuborildi / Xatolik / Jami`) is sent back to the admin. If you add another hidden button/URL to `start()`, decide explicitly whether it should also appear in the broadcast — they intentionally diverge today.
- [helper/start.js](src/bot/helper/start.js) handles the `/start` flow: upserts a `Users` document keyed by `chat_id` and replies with **two** inline `web_app` buttons — the Mini App (`MINI_APP_URL` from env) and a **hardcoded** "Диспетчер платформаси" URL (`https://yukchi-dispetcher.coachingzona.uz/auth`). If that dispatcher host ever changes, this is the only place to update it — it is not read from env.
- [query.js](src/bot/query.js) registers a `callback_query` listener but its body is empty; inline-button callbacks currently no-op after acknowledging.
